...
app.get("/resorts", async (req, res) => {
  try {
    const query = "SELECT * FROM resort where active = true";
...
app.post("/resort", async (req, res) => {
  try {
    ...
    const query =
      "INSERT INTO resort (name, description, city, price, photo) VALUES ($1, $2, $3, $4 + 5, $5) RETURNING *";
...
app.put("/resort/:id", (req, res) => {
  ...
  pool.query(q, [...values, resortid], (err, data) => {
    if (err) return res.json(err)
